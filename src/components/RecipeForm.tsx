...
const RecipeForm: React.FC<RecipeFormElementProps> = (props) => {
  ...
  const [selectedIngredients, setSelectedIngredients] = useState<string[]>(
    props.defaultInputData?.ingredients || []
  );
  const [instructions, setInstructions] = useState<string[]>(
    props.defaultInputData?.instructions || []
  );
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-2">
            {ingredientOptions.map((ingredient) => (
              <motion.label
                key={ingredient}
                className={`flex items-center space-x-2 bg-gray-50 p-3 rounded-xl shadow-sm cursor-pointer border ${
